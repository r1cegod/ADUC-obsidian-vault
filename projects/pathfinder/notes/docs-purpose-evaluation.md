...
> **TL;DR**: Purpose now passes both its stage-local replay and the current Stage 4 `purpose_eval` seam; the final Vietnamese reply keeps the contradiction-rich trade-off intact.
...
A key theme is contract reliability. The doc shows that prompt quality alone was not enough, because the analyst could still fail to emit the required `PROBE:` anchor consistently. That led to stronger Python composition of the final handoff, and the latest Stage 4 replay shows that the final Vietnamese student-facing reply now preserves that same contradiction-rich trade-off through `output_compiler`.
...
- Purpose now also passes the current Stage 4 `purpose_eval` seam, not just the stage-local replay.
